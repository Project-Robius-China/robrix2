...
// wf_final_reviewer is the Codex final-gate agent — it still needs a Matrix account so
// the bridge can invite it into the !mkgroup room (runtime doesn't matter for accounts).
const AGENTS   = (process.env.AGENTS || 'wf_coordinator wf_implementer wf_reviewer wf_final_reviewer').split(/\s+/).filter(Boolean);
