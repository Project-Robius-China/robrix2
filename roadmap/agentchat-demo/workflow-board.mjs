#!/usr/bin/env node
// Workflow Board — a tiny, dependency-free viewer for the demo project's
// issue→spec→plan→implement→review artifacts. Independent of agent-chat (does
// NOT touch its source or dashboard), so both upstreams stay pristine.
//
// It scans the DEMO_REPO (the sandbox) for:
//   specs/project.spec.md, specs/task-*.spec.md
//   issues/NNN-*.md
//   docs/plans/NNN-*.md
//   docs/progress.md, docs/agent-knowledge.md
// and renders them as dark cards (Agent-Monitor style) with click-to-read.
//
// Usage:
//   DEMO_REPO=/path/to/sandbox node workflow-board.mjs       # → http://127.0.0.1:8086
//   PORT=8086 DEMO_REPO=... node workflow-board.mjs

import { createServer } from 'http';
import { readFileSync, readdirSync, statSync } from 'fs';
import { join, basename } from 'path';

const REPO = process.env.DEMO_REPO || '/Users/zhangalex/Work/agentchat-demo-sandbox';
...
function collect() {
  ...
  // task-*.spec.md tagged 'task'). project.spec.md also shows in the Project
  // column above; surfacing it here too matches "the spec is in the project dir".
...
function readArtifact(relId) {
  // relId is repo-relative, restrict to known subdirs to avoid traversal.
  if (!/^(specs|issues|docs)\//.test(relId) || relId.includes('..')) return null;
...
<header>
  ...
  <span class="sub">${esc(REPO)} · issue → spec → plan → implement → review</span>
  <a href="http://127.0.0.1:8084" target="_blank">Agent Monitor →</a>
</header>
<div class="board">${sections.map(col).join('')}</div>
...
async function openDoc(id){
  const r = await fetch('/doc?id='+encodeURIComponent(id));
...
// auto-refresh card list every 5s (cheap, no framework)
setInterval(async()=>{ try{ const r=await fetch('/');
...
const server = createServer((req, res) => {
  ...
  if (u.pathname === '/doc') {
    ...
    const md = readArtifact(id);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify(md == null ? { error: 'not found' } : { html: mdToHtml(md) }));
    return;
  }
  res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
  res.end(page(collect()));
});

server.listen(PORT, '127.0.0.1', () => {
  console.log(`Workflow Board running on http://127.0.0.1:${PORT}  (DEMO_REPO=${REPO})`);
});
