...
/// Actions emitted in response to a [`MatrixRequest::SendCallNotify`].
///
/// The 1:1 orchestrator uses these to confirm the ring landed (or to
/// fall back to `Ended { Cancelled }` if it didn't).
#[derive(Clone, Debug)]
pub enum CallNotifyAction {
    Sent {
        room_id: OwnedRoomId,
        call_id: String,
    },
...
pub enum MatrixRequest {
    ...
    /// Request to send an `m.call.notify` event ringing a single user
    /// for a 1:1 voice call (MSC4075). The 1:1 orchestrator emits this
    /// when the local user clicks the voice-call button in a DM.
    ///
    /// The result of the send arrives as a [`CallNotifyAction`].
    SendCallNotify {
        /// Room in which the ring is sent (a DM, in the 1:1 case).
        room_id: OwnedRoomId,
        /// Stable UUID generated by the caller; the same value is used
        /// in the orchestrator's [`crate::voip::oneonone::OneOnOneCallState`]
        /// to correlate this ring with later session events.
        call_id: String,
        /// User to ring.
        callee: OwnedUserId,
    },
...
async fn matrix_worker_task(
    ...
    while let Some(request) = request_receiver.recv().await {
        match request {
            ...
            MatrixRequest::SendCallNotify { room_id, call_id, callee } => {
                let Some(client) = get_client() else { continue };
                #[allow(deprecated)]
                let _notify_task = Handle::current().spawn(async move {
                    let Some(room) = client.get_room(&room_id) else {
                        error!("Room not found for SendCallNotify: {room_id}");
                        Cx::post_action(CallNotifyAction::Failed {
                    ...
                    let content = crate::voip::ringing::new_oneonone_ring(
                        call_id.clone(),
                        callee.clone(),
                    );
                    log!("Sending m.call.notify ring to {callee} in room {room_id} (call_id={call_id})");
                    match room.send(content).await {
                        Ok(_response) => {
                            Cx::post_action(CallNotifyAction::Sent {
                                room_id,
                                call_id,
                            });
                        }
                        Err(e) => {
                            error!("Failed to send m.call.notify in {room_id}: {e:?}");
                            Cx::post_action(CallNotifyAction::Failed {
                                room_id,
                                call_id,
                                error: format!("{e}"),
...
async fn start_matrix_client_login_and_sync(rt: Handle) {
    ...
    loop {
        let (client, sync_service, logged_in_user_id, client_session) = 'login_loop: loop {
            ...
            // Listen for inbound `m.call.notify` events so the 1:1 voice
            // call ringer can surface incoming rings (MSC4075).
            crate::voip::ringing::register_inbound_handler(client.clone());
