            ...
            main_area := View {
                ...
                content_scroll := ScrollYView {
                    ...
                    addresses_card := RoundedView {
                        ...
                        // A PortalList so every alias gets an actionable row.
                        // Its height is managed by `render_alias_section`.
                        ...
                        add_address_row := View {
                            // Hidden until the alias power-level fetch confirms
                            // that this user may manage room aliases.
                            visible: false
                            width: Fill
                            height: Fit
                            flow: Right
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
        while let Some(widget_to_draw) = self.view.draw_walk(cx, scope, walk).step() {
            ...
            if self.active_tab == SettingsTab::General {
                let count = self.alias_entries.len();
                list.set_item_range(cx, 0, count);
                while let Some(index) = list.next_visible_item(cx) {
                    if let Some(props) = self.alias_entries.get(index).cloned() {
...
impl RoomSettingsModal {
    ...
    /// Apply fetched settings (topic, publication, and join rule) asynchronously.
    ...
    fn render_alias_section(&mut self, cx: &mut Cx) {
        ...
        // The alias-management contract is room-only. Main now reuses this
        // modal for spaces, so keep aliases visible there but never expose the
        // write controls until space alias management is specified separately.
        let can_manage = self.can_manage_aliases && !self.is_space;
...
impl RoomSettingsModalRef {
    ...
    pub fn show_settings(
        ...
        inner.show(cx, room_id, room_name, room_topic, canonical_alias, alias_stage, is_space)
    }
