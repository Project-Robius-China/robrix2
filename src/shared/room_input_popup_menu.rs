    ...
    mod.widgets.RoomInputPopupMenu = set_type_default() do #(RoomInputPopupMenu::register_widget(vm)) {
        ...
        main_content := RoundedShadowView {
            ...
            send_location_button := mod.widgets.RoomInputPopupMenuButton {
                draw_icon.svg: (ICON_LOCATION_PERSON)
                text: "Send current location"
            }

            sticker_button := mod.widgets.RoomInputPopupMenuButton {
...
pub enum RoomInputPopupMenuAction {
    ...
    /// Emitted when the upload photo/video button is clicked.
    UploadPhotoOrVideo,
    /// Emitted when the upload file button is clicked.
    UploadFile,
    /// Emitted when the send location button is clicked.
    SendCurrentLocation,
    /// Emitted when the sticker button is clicked.
    OpenStickers,
    #[default]
    None,
}
...
impl WidgetMatchEvent for RoomInputPopupMenu {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions, _scope: &mut Scope) {
        ...
        } else if self.button(cx, ids!(sticker_button)).clicked(actions) {
...
impl RoomInputPopupMenu {
    ...
    fn reset_button_hover(&mut self, cx: &mut Cx) {
        ...
        self.button(cx, ids!(sticker_button)).reset_hover(cx);
    }
