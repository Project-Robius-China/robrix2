...
这四条中，「人授权」在受管运行时和有效 owner 绑定的前提下由审批协议**强制**（第 5.4 章、第 8 章）；「同一空间」「可介入」由 Matrix 传输与成员关系提供；「人拍板」和主动汇报目前主要是工作流约定。保障强度不同，不能混为一谈。
...
flowchart TB
    ...
    BR <-->|"木偶账号 @ac_&lt;agent&gt;<br/>桥机器人（默认 agent-bridge，<br/>-&lt;user&gt; 后缀为部署约定）"| P
```
