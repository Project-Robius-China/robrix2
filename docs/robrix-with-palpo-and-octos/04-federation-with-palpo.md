...
6. [Cross-Federation Chat Test](#6-cross-federation-chat-test)
...
This document uses the dual-node Docker environment under `moly-ecosystem/palpo-docker/`. Two Palpo nodes discover each other over an internal Docker network (`palpo-federation`), with no public domain needed.
...
Full deployment directory layout:

```
palpo-docker/
├── docker-compose.yml              # 5 services: 2 palpo + 2 postgres + octos
├── Dockerfile                      # Palpo local build image
...
│   │   └── media/                  # Uploaded media (persistent)
│   └── node2/
│       ├── palpo.toml              # server_name = "palpo-2:8448"
│       └── media/
└── octos/
    └── config.json                 # Octos bot config (connects to palpo-1)
```
...
services:
  ...
  # ── Node 2: regular user ──────────────────────────────
  palpo-2:
    image: palpo:local-dev          # Reuse the image built by palpo-1
  ...
  octos:
    build:
      context: ../octos           # Point to your local octos source
...
> 📁 **Actual files:**
> - [`palpo-and-octos-deploy/federation/config/botfather.json`](../../palpo-and-octos-deploy/federation/config/botfather.json) -- Matrix channel profile (core config for connecting to palpo-1)
> - [`palpo-and-octos-deploy/federation/config/octos.json`](../../palpo-and-octos-deploy/federation/config/octos.json) -- Octos runtime config (profiles directory, etc.)

`botfather.json` tells Octos how to connect to palpo-1:
...
Key points:
- `homeserver` and `server_name` **are two different concepts**:
  - `homeserver = "http://palpo-1:8008"` -- URL for Octos to call the C-S API
  - `server_name = "palpo-1:8448"` -- the outward Matrix identity (must match palpo-1's `palpo.toml`)
- The `as_token` / `hs_token` values **must match exactly** those in `octos.yaml`, otherwise Palpo will refuse the connection
- `allowed_senders: []` (empty array) means **all users** -- including federated ones -- can talk to the bot
...
## 6. Cross-Federation Chat Test

### 6.1 Register alice on palpo-2
...
### 6.2 (Optional) Verify federation with curl

Before opening Robrix, use curl to query the bot's profile on palpo-1 via palpo-2 -- this validates the federation channel works:
...
**Expected result:** returns `{"displayname": "...", "avatar_url": "..."}` or an empty object `{}`. If you get `404`, federation is broken -- see [Section 8](#8-troubleshooting).

### 6.3 Log in to palpo-2 from Robrix

Open Robrix. On the login screen:

| Field | Value | Note |
|-------|-------|------|
| **Username** | `@alice:palpo-2:8448` | Full MXID with server_name |
| **Password** | `test1234` | The password from Step 6.1 |
| **Homeserver** | `http://localhost:6002` | HTTP URL, **not** the MXID server_name |

> **⚠️ Easy pitfall:** `palpo-2:8448` inside the username is the Matrix identity (server_name), but the Homeserver URL must be `http://localhost:6002` (the host-exposed C-S port). These are two different things. Robrix sends HTTP requests to the URL but constructs MXIDs from the server_name.
