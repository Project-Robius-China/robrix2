...
| Octos logs show `400 Bad Request: "Invalid max_tokens value, the valid range of max_tokens is [1, 8192]"` (DeepSeek) | Octos's default chat `max_tokens` is 16384, which exceeds DeepSeek's per-response cap (8192). Octos has a model-aware `max_output_tokens("deepseek-chat")` that returns 8000, but it is not wired into `ChatConfig::default()` yet (upstream gap; worth an issue against [octos PR #345](https://github.com/ZhangHanDong/octos/pull/345)). | In `config/botfather.json`, add `"max_output_tokens": 8000` inside `config.gateway` (see [3.5](#35-octos-bot-profile-configbotfatherjson)), then `docker compose restart octos`. No rebuild needed. |
