...
| A2 | **人工验收无法记录**。只能在 commit message 里写"verified by user 2026-08-18"，`stamp` 永远 `Spec-Passing: false` | `agent-spec attest <spec> --scenario <name> --by <who> [--note]` 写入 run log；`explain/stamp/matrix` 显示"attested by X at T"；attest 绑定 spec 内容指纹，spec 改动后失效 | P0 |
...
| B4 | `precedence-fallback-coverage` 对"Keep the signature of X unchanged"这类约束误报 | 启发式排除 "keep/unchanged/do not change" 型约束 | P2 |
...
| C4 | `trace --gate` 依赖 `.agent-spec/runs` 里最近的 lifecycle 结果，CI 干净 checkout 必须先跑 lifecycle | `trace --verify`（按需跑 satisfying spec）或文档写清顺序 | P2 |
