...
本指南配套一个**可直接运行的示例包**（[`example/`](example/)）：复制两个文件、填写 5 个必填值、跑一条命令就能起来。
...
- [ ] **一个兼容的 Octos 可执行文件**（需包含 Matrix user-account channel，即 [Octos PR #1475](https://github.com/octos-org/octos/pull/1475) 或之后的构建）
...
- [ ] 一个供 agent 使用的**非加密私聊或房间** —— 当前 user-account channel 只读取明文 `m.room.message`，不会解密 `m.room.encrypted`（见 [第 6 节](#6-故障排查)）

> **提示：** 「给 bot 专用的账号」是关键。Direct 模式本质是「用一个 Matrix 账号登录并代跑 agent」，所以这个账号会以 bot 身份收发消息 —— 用独立账号，别和你自己的身份混用。

### 2.1 安装兼容的 Octos 二进制

Matrix user-account channel 晚于 `v1.1.0` tag 合入，因此普通的 `v1.1.0` release
二进制无法运行本指南的 profile。在明确包含 PR #1475 的新 release 发布之前，请从
当前 Octos 源码构建，并显式启用 Matrix feature：
...
git merge-base --is-ancestor 355147f1 HEAD || {
  echo "当前 checkout 不包含 Octos PR #1475" >&2
  exit 1
}
...
`git merge-base` 会阻止你误用 feature 合入前的 checkout。后续若有高于 `v1.1.0`
且明确包含 #1475 的 Octos release，可直接使用该 release 二进制。
...
| [`.gitignore`](example/.gitignore) | 防止生成的凭据和运行数据被误提交 |
...
# 1. 生成 profile，编辑其中 4 个账号/权限值
cp myagent.example.json myagent.json
#    改：homeserver、user_id、password、allowed_senders
...
> **需要改 5 个必填值：**`myagent.json` 的 channel 中有 4 个：`homeserver`、`user_id`、`password`，以及 **`allowed_senders`（填你自己的 MXID —— 谁能驱使这个 agent，见 [§4](#4-配置字段详解) 的安全说明）**；第 5 个是 `.env` 中的 `DEEPSEEK_API_KEY`。其余字段（`llm`、`created_at`/`updated_at`、以及为「个人助手」预设好的 `auto_join`/`group_policy`/`require_mention`）保持原样即可 —— 下一节解释它们为什么这么设。
...
- `gateway.max_history` / `max_output_tokens` — 会话行为调优，可选

> **安全提示：** `myagent.json` 里的 `password` 和 `.env` 里的 key 都是明文 —— **别提交进版本库**。示例的 `.gitignore` 会保护标准生成文件名，但提交前仍应检查 `git status`；只提交 `*.example` 模板。若你的 Octos 版本支持，`access_token` 可以避免保存账号密码，但它仍是 bearer secret，必须妥善保护，泄露后应立即撤销。
...
3. 点 **「添加 agent」**
4. **Step 1 of 2 · Choose a framework** —— 选 **「Octos (Direct)」** 那张卡片（Octos 图标，标签「直接 Agent」）
...
| 加密房里没有回复 | 当前 Direct channel 不解密 `m.room.encrypted` 事件 | 新建或改用**非加密**私聊/房间；新发送的加密消息同样无法读取 |
| Robrix 里加不上 | Step 1 选成了 **Octos**（AppService）而非 **Octos (Direct)** | 回退重选 Direct 卡片，Step 2 才会是「Agent Matrix ID + Add friend & bind」 |

> **关于代理陷阱：** 如果 octos 跑在设了全局代理（Clash 等）的机器上，它连**本地或 LAN 上的 homeserver** 时会把 `/sync` 也走代理 → 报 502 → 收不到消息。把 homeserver 的 host/IP 填入 `.env` 的 `MATRIX_NO_PROXY_HOST`；`start.sh` 会在保留已有配置的前提下把它加入 `NO_PROXY`，发往外网 LLM 的请求仍可走代理。

> **关于加密房：** 当前 Direct channel 只处理明文 `m.room.message`，没有实现 Matrix E2EE 解密；新消息和历史消息都无法读取。**Direct agent 必须在非加密房中使用。**
...
*本指南基于 2026 年 7 月的 Octos user-account channel（PR #1475）与 Robrix 的 Octos (Direct) 接入方式编写。Octos `v1.1.0` tag 早于该 feature，请使用之后的构建。配置字段以各项目仓库为准。*
