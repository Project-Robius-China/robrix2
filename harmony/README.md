...
> ⚠️ **这些 makepad 改动目前只改在 cargo 的 git checkout 里**（`~/.cargo/git/checkouts/makepad-*/<rev>/…`），**不是仓库内、也没上游**。持久化方案见 §6.2。
...
### 6.2 makepad 补丁不持久（务必处理）

- §4.3 / §4.4 的 makepad 修改都在 cargo 的 git checkout 里，**`cargo clean` 或升级 makepad 后会丢**。
- 而且一个坑：**cargo 对 git 依赖是按 commit 指纹缓存的，不是按文件 mtime**。所以直接改 checkout 里的文件**不会触发重编**（会静默用旧缓存）；必须 `cargo clean -p <crate> --target aarch64-unknown-linux-ohos` 才会用上你的改动。
- 持久化方案（二选一）：
  1. **上游 PR** 给 makepad（这些是真实的 OHOS 编译/渲染 bug）。
  2. **本地 `[patch]`**：在 robrix `Cargo.toml` 加
     ```toml
     [patch."https://github.com/makepad/makepad"]
     makepad-widgets     = { path = "<本地带修复的 makepad>/widgets" }
     makepad-code-editor = { path = "<本地带修复的 makepad>/code_editor" }
     ```
     其余 makepad crate（platform/draw…）会自动跟着本地 workspace 走。注意本地 clone 的 commit 要和 pin 的 `3d18a137` 兼容（你机器上 `../makepad` 在 `dev`/`3cf602a4`，和 pin 有差异，需评估）。
...
- **已改进仓库（待你测试后提交）**：
  - `Cargo.toml`：`rfd` 排除 ohos。
  - `src/settings/account_settings.rs`、`src/home/create_room.rs`、`src/home/room_settings_modal.rs`、`src/room/room_input_bar.rs`、`src/shared/attachment_download.rs`：rfd 调用点 cfg 收窄（ohos → mobile 桩）。
  - 新增 `harmony/`：`ohos.sh`、`README.md`（本文）、`.gitignore`。
- **不在仓库里（在 cargo checkout，需持久化，见 §6.2）**：makepad 的 4 处补丁 + `MAKEPAD=ohos_sim`（后者是环境变量，已在 ohos.sh）。
