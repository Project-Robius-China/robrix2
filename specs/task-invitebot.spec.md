...
- **邀请结果归属（Codex 补强 + 回归修复）**：`InviteResultAction::Sent/Failed`
  的 toast 仅由 `pending_invited_users` 包含该用户的 RoomScreen 展示（发起
  者拥有反馈）。为使非 picker 的发起点不被静音，闭包型发起点（敲门审批、
  失败重试）与 invite modal 在 submit 同时广播
  `InviteAction::InviteUserRequested`，由同房 RoomScreen 记录 pending；
  成员未就绪时选择器先显示 loading，成员到达后自动重建（Codex）。
...
- 不要把选择器实现为复用 `invite_modal` 的 Modal UI（选择器必须是内联弹窗；
  `invite_modal.rs` 仅允许为邀请结果归属机制补发 `InviteUserRequested` 通知）
