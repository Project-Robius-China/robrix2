...
- 步骤切换(`sync_steps`)时必须把 step1 的 `framework_list` PortalList 复位到 item 0 且清空 scroll offset,并把 step2 的 `step2_view` ScrollYView 复位到顶部;否则 step1 的 4 张 framework 卡溢出固定高度区域后,残留滚动偏移会裁切后续步骤顶部内容
...
场景: 步骤切换时复位滚动到顶部
  ...
  当 检查步骤切换逻辑
  那么 `sync_steps` 函数体同时出现 `set_first_id_and_scroll(0, 0.0)` 与 `step2_view` 的 `set_scroll_pos`
